...
class Captcha(object):
    ...
    def get_text(self):
        captcha_text = ""
        temp = random.choices(self.chars, k = 4)
        for item in temp:
            captcha_text = captcha_text + str(item)
        return captcha_text
        
    def generate(self, fmt='PNG'):
        """生成验证码(文字和图片)"""
        captcha_text = self.get_text()
